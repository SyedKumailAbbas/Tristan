...
import podcast from "../../assets/Group 126.png"; // The image for "PODCAST TESTIMONIALS"
import keynotes from "../../assets/Group 237.png"; // The image for "KEYNOTES"
import { Heading } from "../Heading/heading";

const Testimonials = ({
  title = "TESTIMONIALS",
  videoIds = [],
  seeMore, // { label?: string, onClick?: () => void, href?: string }
}) => {
  const hasSeeMore = Boolean(seeMore && (seeMore.onClick || seeMore.href));
  const seeMoreLabel = seeMore?.label ?? "SEE MORE";

  // Decide which image to show based on the title
  let headingImage;
  if (title === "keynotes") {
    ...
    headingImage = testimonial; // Default image (can be changed to any fallback image)
  }

  // Decide what to render for See More (button or link)
    ...
    <section className="bg-black pt-0 pb-20 relative overflow-hidden max-w-full mx-auto">
      ...
      <div className="relative w-full mb-12">
        <div className="relative">
          <Heading imgSrc={headingImage} altText={title} />
        </div>
      </div>

      {/* Video testimonials grid */}
      <div className="max-w-6xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-40 mb-12">
